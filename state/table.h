...
struct table_callback_data {
  ...
  /** Retry structure containong the remaining number of retries, timeout and a point
 ...
 * @param retry Retry relevant information: retry timeout, number of remaining retries, and retry callback.
 ...
 * - a table command is issued with an associated callback and a callback data structure;
 * - the last timeout associated to this command expires, as a result the callback data structure is freed;
 * - a reply arrives, but now the callback data structure is gone, so we have to ignore this reply;
 *
 * This hash table enables us to ignore such replies. The operations on the hash table are as follows.
 *
 * When we issue a table command we add a new entry to the hash table that is keyed by the address of the callback's
 * data structure.
 *
 * When we receive the reply, we check whether the callback still exists in this hash table, and if not we just ignore
 * the reply.
 *
 * When the last timeout associated to the command expires we remove the entry associated to the collback.
 */
typedef struct {
...
/**
 *
 * @param key The pointer to the data structure of the callback we want to insert.
 * @return None.
 */
...
/**
 *
 * @param key The pointer to the data structure of the callback we are looking for.
...
/**
 *
 * @param key Key, defined as the pointer to the data structure of the callback we want to remove.
 * @return None.
 */
