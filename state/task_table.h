...
/* Callback for getting an entry from the task table. Task spec will be freed by the system
 * after the callback*/
typedef void (*task_table_callback)(task_spec *task, void *context);

/* Get specific task from the task table. */
status task_table_get_task(db_handle *db, task_id task_id, task_table_callback callback, void *context);
