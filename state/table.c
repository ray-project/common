...
table_callback_data *init_table_callback(db_handle *db_handle,
  ...
  cb_data->timer_id = event_loop_add_timer(db_handle->loop,
                                           retry->timeout,
                                           table_timeout_handler,
                                           cb_data);
...
/**
 * List of outstanding callbacks. We need to maintain this list for the case in which a reply is received
 * after te last timeout expires and all relevant data structures are removed. In this case we just need
 * to ignore the reply.
 * */
