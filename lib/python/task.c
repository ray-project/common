...
static int PyTask_init(PyTask *self, PyObject *args, PyObject *kwds) {
  ...
  int num_returns;
  if (!PyArg_ParseTuple(args, "O&Oi", &PyObjectToUniqueID, &function_id, &arguments, &num_returns)) {
    return -1;
  }
  ...
  self->spec = alloc_task_spec(function_id, size, num_returns, value_data_bytes);
  for (size_t i = 0; i < size; ++i) {
    PyObject* arg = PyList_GetItem(arguments, i);
    if (PyObject_IsInstance(arg, (PyObject*)&PyObjectIDType)) {
      task_args_add_ref(self->spec, ((PyObjectID*) arg)->object_id);
    } else {
      PyObject* data = *((PyObject **) utarray_eltptr(val_repr_ptrs, val_repr_index));
      task_args_add_val(self->spec, PyString_AS_STRING(data), PyString_GET_SIZE(data));
      Py_DECREF(data);
      val_repr_index += 1;
    }
  }
...
static PyObject* PyTask_arguments(PyObject *self, PyObject* args) {
  ...
  if (task_arg_type(spec, arg_index) == ARG_BY_REF) {
    ...
    return PyObjectID_make(object_id);
      ...
      PyObject* s = PyMarshal_ReadObjectFromString(task_arg_val(spec, arg_index), task_arg_length(spec, arg_index));
      Py_DECREF(s);
      Py_RETURN_NONE;
  }
}
